...
public sealed class MarketplaceService
{
    ...
    public MarketplaceListing ListTownProperty(ArkoviaTown town, int actorUserId, string assetId, long priceAtomic, TownService towns)
    {
    ...
    public MarketplaceEscrow ReserveForPlayer(string listingId, int buyerUserId, string buyerName, string operationKey, TimeSpan? lifetime = null)
    {
        var buyer = _economy.GetOrCreatePlayer(buyerUserId, buyerName);
        var escrow = GetEscrowAccount();
        lock (_gate)
            return _db.ReserveMarketplaceListing(listingId, "player", buyerUserId.ToString(), buyer.Id, escrow.Id,
    ...
    public MarketplaceSale BuyNowForPlayer(string listingId, int buyerUserId, string buyerName, string operationKey)
    {
        ...
        var salesTaxAtomic = checked((long)Math.Round(
            listing.PriceAtomic * cfg.MarketSalesTaxPercent / 100m, 0, MidpointRounding.AwayFromZero));
        ...
        lock (_gate)
        {
            sale = _db.CompleteTownPropertySale(listingId, held.ReservationKey, escrowAccount.Id,
                buyerUserId, buyerName, salesTaxAtomic, treasury.Id, buyerName);
        }
